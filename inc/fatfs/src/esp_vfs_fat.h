 ...
 * This function also allocates FATFS structure which should be used for esp_f_mount
 * call.
 *
 ...
 * @param[out] out_fs  pointer to FATFS structure which can be used for FATFS esp_f_mount call is returned via this argument.
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_INVALID_STATE if esp_esp_vfs_fat_register was already called
 *      - ESP_ERR_NO_MEM if not enough memory or too many VFSes already registered
 */
esp_err_t esp_esp_vfs_fat_register(const char* base_path, const char* fat_drive,
        size_t max_files, FATFS** out_fs);

/**
 * @brief Un-register FATFS from VFS
 *
 * @note FATFS structure returned by esp_esp_vfs_fat_register is destroyed after
 *       this call. Make sure to call esp_f_mount function to unmount it before
 *       calling esp_esp_vfs_fat_unregister.
 ...
 */
esp_err_t esp_esp_vfs_fat_unregister() __attribute__((deprecated));
 ...
 *                      used when esp_esp_vfs_fat_register was called
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_INVALID_STATE if FATFS is not registered in VFS
 */
esp_err_t esp_esp_vfs_fat_unregister_path(const char* base_path);

/**
 * @brief Configuration arguments for esp_esp_vfs_fat_sdmmc_mount function
 */
typedef struct {
 ...
 * with proper esp_error checking and handling of exceptional conditions.
 *
 ...
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_INVALID_STATE if esp_esp_vfs_fat_sdmmc_mount was already called
 *      - ESP_ERR_NO_MEM if memory can not be allocated
 *      - ESP_FAIL if partition can not be mounted
 *      - other esp_error codes from SDMMC host, SDMMC protocol, or FATFS drivers
 */
esp_err_t esp_esp_vfs_fat_sdmmc_mount(const char* base_path,
...
/**
 * @brief Unmount FAT filesystem and release resources acquired using esp_esp_vfs_fat_sdmmc_mount
 *
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_INVALID_STATE if esp_esp_vfs_fat_sdmmc_mount hasn't been called
 */
esp_err_t esp_esp_vfs_fat_sdmmc_unmount();
