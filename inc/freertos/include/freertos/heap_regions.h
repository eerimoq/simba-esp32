...
void esp_vPortDefineHeapRegionsTagged( const HeapRegionTagged_t * const pxHeapRegions );


/**
 * @brief Allocate memory from a region with a certain tag
 *
 * Like esp_pvPortMalloc, this returns an allocated chunk of memory. This function,
 ...
 *         NULL if unsuccesful.
 */
void *esp_pvPortMallocTagged( size_t xWantedSize, BaseType_t tag );

/**
 * @brief Free memory allocated with esp_pvPortMallocTagged
 *
 * This is basically an implementation of free().
 *
 * @param  pv Pointer to region allocated by esp_pvPortMallocTagged
 */
void esp_vPortFreeTagged( void *pv );
 ...
 *         the program
 */
size_t esp_xPortGetMinimumEverFreeHeapSizeTagged( BaseType_t tag );

/**
 * @brief Get the amount of free bytes in a certain tagged region
 *
 * Works like esp_xPortGetFreeHeapSize but allows the user to specify
 * a specific tag
 * 
 * @param  tag Tag of the memory region
 *
 ...
 */
size_t esp_xPortGetFreeHeapSizeTagged( BaseType_t tag );
