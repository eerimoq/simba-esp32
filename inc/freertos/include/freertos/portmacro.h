...
#define portASSERT_IF_IN_ISR()        esp_vPortAssertIfInISR()
void esp_vPortAssertIfInISR();
...
*/
void esp_vPortCPUInitializeMutex(portMUX_TYPE *mux);
#ifdef CONFIG_FREERTOS_PORTMUX_DEBUG
void esp_vPortCPUAcquireMutex(portMUX_TYPE *mux, const char *function, int line);
portBASE_TYPE esp_vPortCPUReleaseMutex(portMUX_TYPE *mux, const char *function, int line);
void esp_vTaskEnterCritical( portMUX_TYPE *mux, const char *function, int line );
void esp_vTaskExitCritical( portMUX_TYPE *mux, const char *function, int line );
#define portENTER_CRITICAL(mux)        esp_vTaskEnterCritical(mux, __FUNCTION__, __LINE__)
#define portEXIT_CRITICAL(mux)         esp_vTaskExitCritical(mux, __FUNCTION__, __LINE__)
#define portENTER_CRITICAL_ISR(mux)    esp_vPortCPUAcquireMutex(mux, __FUNCTION__, __LINE__)
#define portEXIT_CRITICAL_ISR(mux)    esp_vPortCPUReleaseMutex(mux, __FUNCTION__, __LINE__)
#else
void esp_vTaskExitCritical( portMUX_TYPE *mux );
void esp_vTaskEnterCritical( portMUX_TYPE *mux );
void esp_vPortCPUAcquireMutex(portMUX_TYPE *mux);
portBASE_TYPE esp_vPortCPUReleaseMutex(portMUX_TYPE *mux);
#define portENTER_CRITICAL(mux)        esp_vTaskEnterCritical(mux)
#define portEXIT_CRITICAL(mux)         esp_vTaskExitCritical(mux)
#define portENTER_CRITICAL_ISR(mux)    esp_vPortCPUAcquireMutex(mux)
...
#define portGET_RUN_TIME_COUNTER_VALUE()  esp_xthal_get_ccount()

/* Kernel utilities. */
void esp_vPortYield( void );
void _frxt_setup_switch( void );
#define portYIELD()					esp_vPortYield()
