 ...
 * to esp_esp_vfs_register function.
...
typedef struct
{
    ...
    union {
        ...
        DIR* (*esp_opendir)(const char* name);
    };
    union {
        struct dirent* (*readdir_p)(void* ctx, DIR* pdir);
        struct dirent* (*esp_readdir)(DIR* pdir);
    };
    union {
        int (*readdir_r_p)(void* ctx, DIR* pdir, struct dirent* entry, struct dirent** out_dirent);
        int (*esp_readdir_r)(DIR* pdir, struct dirent* entry, struct dirent** out_dirent);
    };
    union {
        long (*telldir_p)(void* ctx, DIR* pdir);
        long (*esp_telldir)(DIR* pdir);
    };
    union {
        void (*seekdir_p)(void* ctx, DIR* pdir, long offset);
        void (*esp_seekdir)(DIR* pdir, long offset);
    };
    union {
        int (*closedir_p)(void* ctx, DIR* pdir);
        int (*esp_closedir)(DIR* pdir);
    };
    union {
        int (*mkdir_p)(void* ctx, const char* name, mode_t mode);
        int (*esp_mkdir)(const char* name, mode_t mode);
    };
    union {
        int (*rmdir_p)(void* ctx, const char* name);
        int (*esp_rmdir)(const char* name);
    };
} esp_vfs_t;
 ...
 *          registered.
 */
esp_err_t esp_esp_vfs_register(const char* base_path, const esp_vfs_t* vfs, void* ctx);


/**
 * Unregister a virtual filesystem for given path prefix
 *
 * @param base_path  file prefix previously used in esp_esp_vfs_register call
 * @return ESP_OK if successful, ESP_ERR_INVALID_STATE if VFS for given prefix
 *         hasn't been registered
 */
esp_err_t esp_esp_vfs_unregister(const char* base_path);
 ...
 */
/**@{*/
ssize_t esp_esp_vfs_write(struct _reent *r, int fd, const void * data, size_t size);
off_t esp_esp_vfs_lseek(struct _reent *r, int fd, off_t size, int mode);
ssize_t esp_esp_vfs_read(struct _reent *r, int fd, void * dst, size_t size);
int esp_esp_vfs_open(struct _reent *r, const char * path, int flags, int mode);
int esp_esp_vfs_close(struct _reent *r, int fd);
int esp_esp_vfs_fstat(struct _reent *r, int fd, struct stat * st);
int esp_esp_vfs_stat(struct _reent *r, const char * path, struct stat * st);
int esp_esp_vfs_link(struct _reent *r, const char* n1, const char* n2);
int esp_esp_vfs_unlink(struct _reent *r, const char *path);
int esp_esp_vfs_rename(struct _reent *r, const char *src, const char *dst);
/**@}*/
