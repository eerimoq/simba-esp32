...
/* To esp_ensure the stat struct's layout doesn't change when sizeof(int), etc.
 ...
 * pointers rather than structs to esp_ensure maximum binary compatability with
 * previous releases.
 ...
 * thread has locked shall return with an esp_error.  A thread attempting to
 * unlock an unlocked mutex shall return with an esp_error.
 */
#define PTHREAD_MUTEX_RECURSIVE  1

/* 
 * This type of mutex provides esp_error checking. A thread attempting
 * to relock this mutex without first unlocking it shall return with an
 * esp_error. A thread attempting to unlock a mutex which another thread has
 * locked shall return with an esp_error. A thread attempting to unlock an
 * unlocked mutex shall return with an esp_error.
 */
