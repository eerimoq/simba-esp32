 ...
 * @return
 *      - ESP_OK on success
 *      - One of the esp_error codes from SDMMC host controller
 */
esp_err_t esp_sdmmc_card_init(const sdmmc_host_t* host,
        sdmmc_card_t* out_card);

/**
 * @brief Print information about the card to a stream
 * @param stream  stream obtained using fopen or fdopen
 * @param card  card information structure initialized using esp_sdmmc_card_init
 */
void esp_sdmmc_card_print_info(FILE* stream, const sdmmc_card_t* card);

/**
 * Write given number of sectors to SD/MMC card
 *
 * @param card  pointer to card information structure previously initialized using esp_sdmmc_card_init
 ...
 *      - One of the esp_error codes from SDMMC host controller
 */
esp_err_t esp_sdmmc_write_sectors(sdmmc_card_t* card, const void* src,
 ...
 *      - One of the esp_error codes from SDMMC host controller
 */
esp_err_t esp_sdmmc_read_sectors(sdmmc_card_t* card, void* dst,
        size_t start_sector, size_t sector_count);
