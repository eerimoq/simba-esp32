 ...
 *
 */
void esp_spi_flash_init();

/**
 * @brief  Get flash chip size, as set in binary image header
 *
 * @note This value does not necessarily esp_match real flash size.
 *
 * @return size of flash chip, in bytes
 */
size_t esp_spi_flash_get_chip_size();
 ...
 * @return esp_err_t
 */
esp_err_t esp_spi_flash_erase_sector(size_t sector);
 ...
 * @return esp_err_t
 */
esp_err_t esp_spi_flash_erase_range(size_t start_address, size_t size);
 ...
 * @return esp_err_t
 */
esp_err_t esp_spi_flash_write(size_t dest_addr, const void *src, size_t size);
 ...
 * @return esp_err_t
 */
esp_err_t esp_spi_flash_write_encrypted(size_t dest_addr, const void *src, size_t size);
 ...
 * @return esp_err_t
 */
esp_err_t esp_spi_flash_read(size_t src_addr, void *dest, size_t size);
 ...
 * If flash encryption is not enabled, this function behaves the same as esp_spi_flash_read().
 *
 ...
 * @return esp_err_t
 */
esp_err_t esp_spi_flash_read_encrypted(size_t src, void *dest, size_t size);
...
/**
 * @brief Opaque handle for memory region obtained from esp_spi_flash_mmap.
 */
 ...
 * page allocation, use esp_spi_flash_mmap_dump function.
 *
 ...
 */
esp_err_t esp_spi_flash_mmap(size_t src_addr, size_t size, spi_flash_mmap_memory_t memory,
                         const void** out_ptr, spi_flash_mmap_handle_t* out_handle);

/**
 * @brief Release region previously obtained using esp_spi_flash_mmap
 *
 ...
 *
 * @param handle  Handle obtained from esp_spi_flash_mmap
 */
void esp_spi_flash_munmap(spi_flash_mmap_handle_t handle);

/**
 * @brief Display information about mapped regions
 *
 * This function lists handles obtained using esp_spi_flash_mmap, along with range
 * of pages allocated to each handle. It also lists all non-zero entries of
 * MMU table and corresponding reference counts.
 */
void esp_spi_flash_mmap_dump();


#define SPI_FLASH_CACHE2PHYS_FAIL UINT32_MAX /*<! Result from esp_spi_flash_cache2phys() if flash cache address is invalid */

/**
 * @brief Given a memory address where flash is mapped, return the corresponding physical flash offset.
 *
 * Cache address does not have have been assigned via esp_spi_flash_mmap(), any address in flash map space can be looked up.
 *
 ...
 */
size_t esp_spi_flash_cache2phys(const void *cached);

/** @brief Given a physical offset in flash, return the address where it is mapped in the memory space.
 *
 * Physical address does not have to have been assigned via esp_spi_flash_mmap(), any address in flash can be looked up.
 *
 ...
 */
const void *esp_spi_flash_phys2cache(size_t phys_offs, spi_flash_mmap_memory_t memory);
 ...
 */
bool esp_spi_flash_cache_enabled();
 ...
 */
void esp_spi_flash_guard_set(const spi_flash_guard_funcs_t* funcs);
