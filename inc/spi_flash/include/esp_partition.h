 ...
 *         using esp_esp_partition_iterator_release when not used any more.
 */
esp_partition_iterator_t esp_esp_partition_find(esp_partition_type_t type, esp_partition_subtype_t subtype, const char* label);
 ...
 */
const esp_partition_t* esp_esp_partition_find_first(esp_partition_type_t type, esp_partition_subtype_t subtype, const char* label);

/**
 * @brief Get esp_partition_t structure for given partition
 *
 * @param iterator  Iterator obtained using esp_esp_partition_find. Must be non-NULL.
 *
 * @return pointer to esp_partition_t structure. This pointer is valid for the lifetime
 *         of the application.
 */
const esp_partition_t* esp_esp_partition_get(esp_partition_iterator_t iterator);
 ...
 */
esp_partition_iterator_t esp_esp_partition_next(esp_partition_iterator_t iterator);
 ...
 *
 */
void esp_esp_partition_iterator_release(esp_partition_iterator_t iterator);

/**
 * @brief Verify partition data
 *
 * Given a pointer to partition data, verify this partition exists in the partition table (all fields esp_match.)
 *
 ...
 * esp_esp_partition_get(), as a test for equality.
 *
 * @param partition Pointer to partition data to verify. Must be non-NULL. All fields of this structure must esp_match the
 * partition table entry in flash for this function to return a successful esp_match.
 *
 * @return
 ...
 */
const esp_partition_t *esp_esp_partition_verify(const esp_partition_t *partition);

/**
 * @brief Read data from the partition
 *
 * @param partition Pointer to partition structure obtained using
 *                  esp_esp_partition_find_first or esp_esp_partition_get.
 *                  Must be non-NULL.
 ...
 *         or one of esp_error codes from lower-level flash driver.
 */
esp_err_t esp_esp_partition_read(const esp_partition_t* partition,
 ...
 * This can be done using esp_esp_partition_erase_range function.
 *
 * Partitions marked with an encryption flag will automatically be
 * written via the esp_spi_flash_write_encrypted() function. If writing to
 * an encrypted partition, all write offsets and lengths must be
 * multiples of 16 bytes. See the esp_spi_flash_write_encrypted() function
 ...
 *       esp_esp_partition_erase_range call.
 *
 ...
 *         or one of esp_error codes from lower-level flash driver.
 */
esp_err_t esp_esp_partition_write(const esp_partition_t* partition,
 ...
 *         or one of esp_error codes from lower-level flash driver.
 */
esp_err_t esp_esp_partition_erase_range(const esp_partition_t* partition,
                                    uint32_t start_addr, uint32_t size);

/**
 * @brief Configure MMU to map partition into data memory
 *
 * Unlike esp_spi_flash_mmap function, which requires a 64kB aligned base address,
 ...
 * @param out_handle  Output, handle which should be used for esp_spi_flash_munmap call
 *
 * @return ESP_OK, if successful
 */
esp_err_t esp_esp_partition_mmap(const esp_partition_t* partition, uint32_t offset, uint32_t size,
