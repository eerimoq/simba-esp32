...
void esp_spi_flash_init_lock();

// Take mutex protecting access to spi_flash_* APIs
void esp_spi_flash_op_lock();

// Release said mutex
void esp_spi_flash_op_unlock();
...
void esp_spi_flash_disable_interrupts_caches_and_other_cpu();

// Enable cache, enable interrupts (to be added in future), resume scheduler
void esp_spi_flash_enable_interrupts_caches_and_other_cpu();
...
void esp_spi_flash_disable_interrupts_caches_and_other_cpu_no_os();

// Enable cache, enable interrupts on current CPU.
// This function is implied to be called when other CPU is not running or running code from IRAM.
void esp_spi_flash_enable_interrupts_caches_no_os();
...
//
// Only call this while holding esp_spi_flash_op_lock()
void esp_spi_flash_mark_modified_region(uint32_t start_addr, uint32_t length);
