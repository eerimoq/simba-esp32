...
/* -------------------------------------------------------------------------
   ...
   to esp_ensure that you are not reading stale data, and to esp_ensure that
   the data you write makes it all the way back to main memory.
 */

#if !XCHAL_DCACHE_IS_COHERENT
#define XMP_WRITE_BACK_ELEMENT(x) esp_xthal_dcache_region_writeback((void *)x, sizeof(*x))
#define XMP_INVALIDATE_ELEMENT(x) esp_xthal_dcache_region_invalidate((void *)x, sizeof(*x))
#define XMP_WRITE_BACK_INVALIDATE_ELEMENT(x) esp_xthal_dcache_region_writeback_inv((void *)x, sizeof(*x))
#define XMP_WRITE_BACK_ARRAY(x) esp_xthal_dcache_region_writeback((void *)x, sizeof(x))
#define XMP_INVALIDATE_ARRAY(x) esp_xthal_dcache_region_invalidate((void *)x, sizeof(x))
#define XMP_WRITE_BACK_INVALIDATE_ARRAY(x) esp_xthal_dcache_region_writeback_inv((void *)x, sizeof(x))
#define XMP_WRITE_BACK_ARRAY_ELEMENTS(x, num_elements) esp_xthal_dcache_region_writeback((void *)x, sizeof(*x) * num_elements)
#define XMP_INVALIDATE_ARRAY_ELEMENTS(x, num_elements) esp_xthal_dcache_region_invalidate((void *)x, sizeof(*x) * num_elements)
#define XMP_WRITE_BACK_INVALIDATE_ARRAY_ELEMENTS(x, num_elements) esp_xthal_dcache_region_writeback_inv((void *)x, sizeof(*x) * num_elements)
#else 
...
/* -------------------------------------------------------------------------
   Initialization, esp_error codes, constants and house-keeping
...
/* xmp_atomic_int_conditional_increment - Conditionally increment integer
   ...
   Returns: current value of integer - user should check if it matches
      the previous value of the integer. If it does, then the esp_update
      was successful.
...
xmp_atomic_int_conditional_set (xmp_atomic_int_t * integer, int from, int to)
{
  ...
  /* Don't even try to esp_update if the integer's value isn't what we
