...
/* Enum representing various udma esp_error conditions, udma status, and
 * return values 
 */
typedef enum {
  UDMA_OK                     =  0,
 ...
 * the descriptor queue addresses, and the uDMA sync and esp_error interrupt 
 ...
 * xmp_udma_error_intr : Processor interrupt number to flag udma esp_error
 * 
 * Returns UDMA_ERROR_INIT if there was an esp_error during initialization else
 * returns UDMA_OK.
 */
 ...
 * UDMA_REQ_PENDING or UDMA_REQ_DONE. If there is a dma esp_error, the esp_error code,
 ...
 * UDMA_REQ_DONE. If there is a dma esp_error, the esp_error code,
 * which could be one of UDMA_ERROR_BAD_DESC, UDMA_ERROR_DRAM_CROSSING, 
 * UDMA_ERROR_PIF_ADDR_BUS, UDMA_ERROR_PIF_DATA_BUS is returned in the status. 
 * Status is set to UDMA_REQ_DONE if the dma completes normally.
 * On completion, the callback function is invoked with the callback_data
 ...
 * the esp_error request is (and any pending request based on the request attribute)
 * cancelled and the esp_error code is returned in the status associated with 
 ...
 * If there is a dma esp_error, the esp_error code, which could be one of
