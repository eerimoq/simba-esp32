...
/**
  ...
  * @return others : fail
  */
esp_err_t esp_esp_event_loop_init(system_event_cb_t cb, void *ctx);
...
/**
  ...
  */
system_event_cb_t esp_esp_event_loop_set_cb(system_event_cb_t cb, void *ctx);

/**
  * @brief  Get the queue used by event loop
  *
  * @attention : currently this API is used to initialize "q" parameter
  * of esp_wifi_init structure.
  *
  * @return QueueHandle_t : event queue handle
  */
QueueHandle_t esp_esp_event_loop_get_queue(void);
