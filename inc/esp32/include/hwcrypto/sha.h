 ...
 * full SHA API such as the one in mbedtls, and also a basic SHA function esp_esp_sha().
 *
 ...
 */
void esp_esp_sha(esp_sha_type sha_type, const unsigned char *input, size_t ilen, unsigned char *output);
 ...
 * algorithm.
 *
 * @note Call esp_esp_sha_try_lock_engine() before calling this
 ...
 * directly in another way, call esp_esp_sha_wait_idle() after calling this
 * function but before accessing the SHA registers.
 */
void esp_esp_sha_block(esp_sha_type sha_type, const void *data_block, bool is_first_block);
 ...
 *
 */
void esp_esp_sha_read_digest_state(esp_sha_type sha_type, void *digest_state);
 ...
 * esp_esp_sha_try_lock_engine() and failing over to software SHA.
 */
void esp_esp_sha_lock_engine(esp_sha_type sha_type);
 ...
 */
bool esp_esp_sha_try_lock_engine(esp_sha_type sha_type);

/**
 * @brief Unlock an engine previously locked with esp_esp_sha_lock_engine() or esp_esp_sha_try_lock_engine()
 *
 * @param sha_type Type of engine to release.
 */
void esp_esp_sha_unlock_engine(esp_sha_type sha_type);
 ...
 * while it is in use by the SHA engine. Caller should use esp_esp_sha_wait_idle()
 * to esp_ensure the SHA engine is not reading from the memory block in hardware.
 *
 * @note You do not need to lock the memory block before calling esp_esp_sha_block() or esp_esp_sha_read_digest_state(), these functions handle memory block locking internally.
 *
 * Call esp_esp_sha_unlock_memory_block() when done.
 */
void esp_esp_sha_lock_memory_block(void);
 ...
 * Caller should have already locked a SHA engine before calling this function.
 *
 * Call following esp_esp_sha_lock_memory_block().
 */
void esp_esp_sha_unlock_memory_block(void);

/** @brief Wait for the SHA engine to finish any current operation
 *
 * @note This function does not esp_ensure exclusive access to any SHA
 * engine. Caller should use esp_esp_sha_try_lock_engine() and
 * esp_esp_sha_lock_memory_block() as required.
 *
