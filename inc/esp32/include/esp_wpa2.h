...
/**
  ...
  *    - ESP_ERR_WIFI_NO_MEM: fail(internal memory malloc fail)
  */
esp_err_t esp_esp_wifi_sta_wpa2_ent_set_identity(unsigned char *identity, int len);

/**
  * @brief  Clear identity for PEAP/TTLS method.
  */
void esp_esp_wifi_sta_wpa2_ent_clear_identity(void);
...
/**
  ...
  * @attention 2. The new password is used to substitute the old password when eap-mschapv2 failure request message with esp_error code ERROR_PASSWD_EXPIRED is received.
  *
  * @param  password: point to address where stores the password;
