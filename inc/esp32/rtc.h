 ...
 *
 * For now, only esp_esp_deep_sleep function declared in esp_deepsleep.h
 ...
 */
void esp_rtc_init_lite(xtal_freq_t xtal_freq);

/**
 * Switch CPU frequency
 * @param cpu_freq  new CPU frequency
 */
void esp_rtc_set_cpu_freq(cpu_freq_t cpu_freq);
 ...
 */
uint32_t esp_rtc_slowck_cali(cali_clk_t cali_clk, uint32_t slow_clk_cycles);
 ...
 * @param slow_clk_period  Period of slow clock in microseconds, Q13.19 fixed point format (as returned by esp_rtc_slowck_cali).
 ...
 */
void esp_rtc_usec2rtc(uint32_t time_in_us_h, uint32_t time_in_us_l, uint32_t slow_clk_period, uint32_t *cylces_h, uint32_t *cycles_l);
 ...
 */
void esp_rtc_slp_prep_lite(uint32_t deep_slp, uint32_t cpu_lp_mode);
 ...
 * @return TBD
 */
uint32_t esp_rtc_sleep(uint32_t cycles_h, uint32_t cycles_l, uint32_t wakeup_opt, uint32_t reject_opt);

/**
 * @brief Shutdown PHY and RF. TODO: convert this function to another one.
 */
void esp_phy_close_rf(void);
