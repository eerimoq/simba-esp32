...
/**
 * @brief Configuration parameters of LEDC channel for esp_ledc_channel_config function
 */
typedef struct {
...
/**
 * @brief Configuration parameters of LEDC Timer timer for esp_ledc_timer_config function
 */
typedef struct {
    ledc_mode_t speed_mode;         /*!< LEDC speed speed_mode, high-speed mode or low-speed mode*/
 ...
 */
esp_err_t esp_ledc_channel_config(ledc_channel_config_t* ledc_conf);
 ...
 */
esp_err_t esp_ledc_timer_config(ledc_timer_config_t* timer_conf);

/**
 * @brief LEDC esp_update channel parameters
 *        Call this function to activate the LEDC updated parameters.
 *        After esp_ledc_set_duty, esp_ledc_set_fade, we need to call this function to esp_update the settings.
 *
 ...
 *
 */
esp_err_t esp_ledc_update_duty(ledc_mode_t speed_mode, ledc_channel_t channel);
 ...
 *     - ESP_ERR_INVALID_ARG Parameter esp_error
 */
esp_err_t esp_ledc_stop(ledc_mode_t speed_mode, ledc_channel_t channel, uint32_t idle_level);
 ...
 *     - ESP_FAIL Can not find a proper pre-divider number base on the given frequency and the current bit_num.
 */
esp_err_t esp_ledc_set_freq(ledc_mode_t speed_mode, ledc_timer_t timer_num, uint32_t freq_hz);
 ...
 */
uint32_t esp_ledc_get_freq(ledc_mode_t speed_mode, ledc_timer_t timer_num);

/**
 * @brief LEDC set duty
 *        Only after calling esp_ledc_update_duty will the duty esp_update.
 *
 * @param speed_mode Select the LEDC speed_mode, high-speed mode and low-speed mode
 * @param channel LEDC channel(0-7), select from ledc_channel_t
 ...
 *     - ESP_ERR_INVALID_ARG Parameter esp_error
 */
esp_err_t esp_ledc_set_duty(ledc_mode_t speed_mode, ledc_channel_t channel, uint32_t duty);
 ...
 */
int esp_ledc_get_duty(ledc_mode_t speed_mode, ledc_channel_t channel);

/**
 * @brief LEDC set gradient
 *        Set LEDC gradient, After the function calls the esp_ledc_update_duty function, the function can take effect.
 *
 * @param  speed_mode Select the LEDC speed_mode, high-speed mode and low-speed mode
 * @param  channel LEDC channel(0-7), select from ledc_channel_t
 ...
 *     - ESP_ERR_INVALID_ARG Parameter esp_error
 */
esp_err_t esp_ledc_set_fade(ledc_mode_t speed_mode, ledc_channel_t channel, uint32_t duty, ledc_duty_direction_t gradule_direction,
 ...
 */
esp_err_t esp_ledc_isr_register(void (*fn)(void*), void * arg, int intr_alloc_flags, ledc_isr_handle_t *handle);
 ...
 */
esp_err_t esp_ledc_timer_set(ledc_mode_t speed_mode, ledc_timer_t timer_sel, uint32_t div_num, uint32_t bit_num, ledc_clk_src_t clk_src);
 ...
 *     - ESP_OK Success
 */
esp_err_t esp_ledc_timer_rst(ledc_mode_t speed_mode, uint32_t timer_sel);
 ...
 *
 */
esp_err_t esp_ledc_timer_pause(ledc_mode_t speed_mode, uint32_t timer_sel);
 ...
 *     - ESP_OK Success
 */
esp_err_t esp_ledc_timer_resume(ledc_mode_t speed_mode, uint32_t timer_sel);
 ...
 *     - ESP_OK Success
 */
esp_err_t esp_ledc_bind_channel_timer(ledc_mode_t speed_mode, uint32_t channel, uint32_t timer_idx);

/**
 * @brief Set LEDC fade function. Should call esp_ledc_fade_func_install() before calling this function.
 *        Call esp_ledc_fade_start() after this to start fading.
 *
 * @param speed_mode Select the LEDC speed_mode, high-speed mode and low-speed mode,
 * @param channel LEDC channel index(0-7), select from ledc_channel_t
 ...
 *     - ESP_FAIL Fade function init esp_error
 */
esp_err_t esp_ledc_set_fade_with_step(ledc_mode_t speed_mode, ledc_channel_t channel, int target_duty, int scale, int cycle_num);

/**
 * @brief Set LEDC fade function, with a limited time. Should call esp_ledc_fade_func_install() before calling this function.
 *        Call esp_ledc_fade_start() after this to start fading.
 *
 * @param speed_mode Select the LEDC speed_mode, high-speed mode and low-speed mode,
 * @param channel LEDC channel index(0-7), select from ledc_channel_t
 ...
 *     - ESP_FAIL Fade function init esp_error
 */
esp_err_t esp_ledc_set_fade_with_time(ledc_mode_t speed_mode, ledc_channel_t channel, int target_duty, int max_fade_time_ms);
 ...
 */
esp_err_t esp_ledc_fade_func_install(int intr_alloc_flags);

/**
 * @brief Uninstall LEDC fade function.
 *
 */
void esp_ledc_fade_func_uninstall();
 ...
 *     - ESP_ERR_INVALID_ARG Parameter esp_error.
 */
esp_err_t esp_ledc_fade_start(ledc_mode_t speed_mode, ledc_channel_t channel, ledc_fade_mode_t wait_done);
