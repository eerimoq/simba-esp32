 ...
 * @brief      LEDC set duty
 *
 * Set LEDC duty, After the function calls the esp_ledc_update_duty function, the function can take effect.
 *
 * @param speed_mode Select the LEDC speed_mode, high-speed mode and low-speed mode, now we only support high-speed mode. We will access low-speed mode in next version
 *
 * @param channel LEDC channel(0-7), select from ledc_channel_t
 ...
 *     - ESP_ERR_INVALID_ARG Parameter esp_error
 */
esp_err_t esp_ledc_set_fade(ledc_mode_t speed_mode, uint32_t channel, uint32_t duty, ledc_duty_direction_t gradule_direction,
 ...
 * esp_periph_module_enable(PERIPH_LEDC_MODULE);
 * @endcode
 *
 * @code{c}
 * //2. set LEDC timer
 ...
 * };
 * esp_ledc_timer_config(&timer_conf);                          //setup timer.
 * @endcode
 *
 * @code{c}
 * //3. set LEDC channel
 ...
 * }
 * esp_ledc_channel_config(&ledc_conf);                         //setup the configuration
 *
 ...
 * esp_ledc_update_duty(LEDC_HIGH_SPEED_MODE, ledc_channel);    //after set duty, we need to call esp_ledc_update_duty to esp_update the settings.
 * @endcode
 *
 ...
 * esp_ledc_isr_register(ledc_isr_handler, NULL, 0);           //hook the isr handler for LEDC interrupt
 * @endcode
 *
