...
typedef struct {
    ...
    uint8_t dummy_bits;             ///< Amount of dummy bits to esp_insert between address and data phase
    ...
    int queue_size;                 ///< Transaction queue size. This sets how many transactions can be 'in the air' (queued using esp_spi_device_queue_trans but not yet finished using esp_spi_device_get_trans_result) at the same time
 ...
 *         - ESP_OK                on success
 */
esp_err_t esp_spi_bus_initialize(spi_host_device_t host, spi_bus_config_t *bus_config, int dma_chan);
 ...
 *         - ESP_OK                on success
 */
esp_err_t esp_spi_bus_free(spi_host_device_t host);
 ...
 *         - ESP_OK                on success
 */ 
esp_err_t esp_spi_bus_add_device(spi_host_device_t host, spi_device_interface_config_t *dev_config, spi_device_handle_t *handle);
 ...
 *         - ESP_OK                on success
 */
esp_err_t esp_spi_bus_remove_device(spi_device_handle_t handle);
 ...
 *         - ESP_OK                on success
 */
esp_err_t esp_spi_device_queue_trans(spi_device_handle_t handle, spi_transaction_t *trans_desc, TickType_t ticks_to_wait);
 ...
 * esp_spi_device_queue_trans) has succesfully completed. It will then return the description of the
 ...
 *         - ESP_OK                on success
 */
esp_err_t esp_spi_device_get_trans_result(spi_device_handle_t handle, spi_transaction_t **trans_desc, TickType_t ticks_to_wait);


/**
 * @brief Do a SPI transaction
 *
 * Essentially does the same as esp_spi_device_queue_trans followed by esp_spi_device_get_trans_result. Do
 * not use this when there is still a transaction queued that hasn't been finalized 
 * using esp_spi_device_get_trans_result.
 *
 * @param handle Device handle obtained using spi_host_add_dev
 * @param trans_desc Pointer to variable able to contain a pointer to the description of the 
 *                   transaction that is executed
 ...
 *         - ESP_OK                on success
 */
esp_err_t esp_spi_device_transmit(spi_device_handle_t handle, spi_transaction_t *trans_desc);
