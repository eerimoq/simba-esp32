 ...
 */
esp_err_t esp_adc1_config_width(adc_bits_width_t width_bit);
 ...
 *     - ESP_ERR_INVALID_ARG Parameter esp_error
 */
esp_err_t esp_adc1_config_channel_atten(adc1_channel_t channel, adc_atten_t atten);
 ...
 */
int esp_adc1_get_voltage(adc1_channel_t channel);

/**
 * @brief Hall  Sensor output value.
 *        @note
 *        The Hall Sensor uses Channel_0 and Channel_3 of ADC1.
 *        So, firstly: please configure ADC1 module by calling esp_adc1_config_width before calling esp_hall_sensor_read.
                 We recommend that the WIDTH ADC1 be configured as 12Bit, because the values of esp_hall_sensor_read are small and almost the same if WIDTH ADC1 is configured as 9Bit, 10Bit or 11Bit.
 ...
 */
int esp_hall_sensor_read();

/**
 *----------EXAMPLE TO USE ADC1------------ *
 * @code{c}
 *     esp_adc1_config_width(ADC_WIDTH_12Bit);//config adc1 width
 *     esp_adc1_config_channel_atten(ADC1_CHANNEL_0,ADC_ATTEN_0db);//config channel0 attenuation
 *     int val=esp_adc1_get_voltage(ADC1_CHANNEL_0);//get the  val of channel0
 * @endcode
 **/
