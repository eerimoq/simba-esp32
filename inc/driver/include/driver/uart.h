 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_uart_isr_register(uart_port_t uart_num, void (*fn)(void*), void * arg, int intr_alloc_flags,  uart_isr_handle_t *handle);
 ...
 * @param  intr_alloc_flags Flags used to allocate the interrupt. One or multiple (ORred)
 *            ESP_INTR_FLAG_* values. See esp_esp_intr_alloc.h for more info. Do not set ESP_INTR_FLAG_IRAM here
 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_uart_driver_install(uart_port_t uart_num, int rx_buffer_size, int tx_buffer_size, int queue_size, QueueHandle_t* uart_queue, int intr_alloc_flags);
 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_uart_get_buffered_data_len(uart_port_t uart_num, size_t* size);
 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_uart_disable_pattern_det_intr(uart_port_t uart_num);
 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_uart_enable_pattern_det_intr(uart_port_t uart_num, char pattern_chr, uint8_t chr_num, int chr_tout, int post_idle, int pre_idle);
/***************************EXAMPLE**********************************
 *
 *
 ...
 *     esp_uart_driver_install(uart_num, 1024 * 2, 1024*4, 0, NULL, 0);
 *     uint8_t data[1000];
 *     while(1) {
 *         //Read data from UART
