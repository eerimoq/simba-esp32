...
#define SDMMC_HOST_DEFAULT() {\
    ...
    .io_voltage = 3.3f, \
    .init = &esp_sdmmc_host_init, \
    .set_bus_width = &esp_sdmmc_host_set_bus_width, \
    .set_card_clk = &esp_sdmmc_host_set_card_clk, \
    .do_transaction = &esp_sdmmc_host_do_transaction, \
    .deinit = &esp_sdmmc_host_deinit, \
}
 ...
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_INVALID_STATE if esp_sdmmc_host_init was already called
 *      - ESP_ERR_NO_MEM if memory can not be allocated
 */
esp_err_t esp_sdmmc_host_init();
 ...
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_INVALID_STATE if host has not been initialized using esp_sdmmc_host_init
 */
esp_err_t esp_sdmmc_host_init_slot(int slot, const sdmmc_slot_config_t* slot_config);
 ...
 */
esp_err_t esp_sdmmc_host_set_bus_width(int slot, size_t width);
 ...
 * @return
 *      - ESP_OK on success
 *      - other esp_error codes may be returned in the future
 */
esp_err_t esp_sdmmc_host_set_card_clk(int slot, uint32_t freq_khz);
 ...
 *       can call esp_sdmmc_host_do_transaction as long as other sdmmc_host_*
 ...
 */
esp_err_t esp_sdmmc_host_do_transaction(int slot, sdmmc_command_t* cmdinfo);
 ...
 * @return
 *      - ESP_OK on success
 *      - ESP_ERR_INVALID_STATE if esp_sdmmc_host_init function has not been called
 */
esp_err_t esp_sdmmc_host_deinit();
