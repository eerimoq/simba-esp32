...
/**
 * @brief I2S configuration parameters for esp_i2s_param_config function
 *
 */
typedef struct {
    ...
    int                     intr_alloc_flags;       /*!< Flags used to allocate the interrupt. One or multiple (ORred) ESP_INTR_FLAG_* values. See esp_esp_intr_alloc.h for more info */
...
/**
 * @brief I2S pin number for esp_i2s_set_pin
 *
 */
typedef struct {
 ...
 */
esp_err_t esp_i2s_set_pin(i2s_port_t i2s_num, const i2s_pin_config_t *pin);
 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_i2s_driver_install(i2s_port_t i2s_num, const i2s_config_t *i2s_config, int queue_size, void* i2s_queue);
 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_i2s_driver_uninstall(i2s_port_t i2s_num);
 ...
 *
 * @return  Number of bytes written, or ESP_FAIL (-1) for parameter esp_error. If a timeout occurred, bytes written will be less than total size.
 */
int esp_i2s_write_bytes(i2s_port_t i2s_num, const char *src, size_t size, TickType_t ticks_to_wait);
 ...
 * configuration (see i2s_config_t).
 *
 * @return  Number of bytes read, or ESP_FAIL (-1) for parameter esp_error. If a timeout occurred, bytes read will be less than total size.
 */
int esp_i2s_read_bytes(i2s_port_t i2s_num, char* dest, size_t size, TickType_t ticks_to_wait);
 ...
 *
 * @return Number of bytes successfully pushed to DMA buffer, or ESP_FAIL (-1) for parameter esp_error. Will be either zero or the size of configured sample buffer.
 */
int esp_i2s_push_sample(i2s_port_t i2s_num, const char *sample, TickType_t ticks_to_wait);
 ...
 *
 * @return Number of bytes successfully read from DMA buffer, or ESP_FAIL (-1) for parameter esp_error. Byte count will be either zero or the size of the configured sample buffer.

 */
int esp_i2s_pop_sample(i2s_port_t i2s_num, char *sample, TickType_t ticks_to_wait);
 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_i2s_set_sample_rates(i2s_port_t i2s_num, uint32_t rate);

/**
 * @brief Stop I2S driver
 *
 * Disables I2S TX/RX, until esp_i2s_start() is called.
 *
 * @param i2s_num  I2S_NUM_0, I2S_NUM_1
 *
 * @return
 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_i2s_stop(i2s_port_t i2s_num);

/**
 * @brief Start I2S driver
 *
 * It is not necessary to call this function after esp_i2s_driver_install() (it is started automatically), however it is necessary to call it after esp_i2s_stop().
 *
 *
 * @param i2s_num  I2S_NUM_0, I2S_NUM_1
 *
 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_i2s_start(i2s_port_t i2s_num);
 ...
 *     - ESP_FAIL Parameter esp_error
 */
esp_err_t esp_i2s_zero_dma_buffer(i2s_port_t i2s_num);
