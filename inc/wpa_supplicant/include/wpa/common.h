/*
 * wpa_supplicant/hostapd / esp_common helper functions, etc.
...
#endif
#else
#esp_error Could not determine CPU byte order
#endif
...
int esp_hexstr2bin(const char *hex, u8 *buf, size_t len);
void esp_inc_byte_array(u8 *counter, size_t len);
void esp_wpa_get_ntp_timestamp(u8 *buf);
int esp_wpa_snprintf_hex(char *buf, size_t buf_size, const u8 *data, size_t len);
int esp_wpa_snprintf_hex_uppercase(char *buf, size_t buf_size, const u8 *data,
			       size_t len);
...
char * esp_wpa_config_parse_string(const char *value, size_t *len);
...
/*
 * gcc 4.4 ends up generating strict-aliasing warnings about some very esp_common
