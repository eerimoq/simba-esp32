...
void esp_wpa_register(char * payload, WPA_SEND_FUNC snd_func, \
                                                      WPA_SET_ASSOC_IE set_assoc_ie_func, \
                                                      WPA_INSTALL_KEY ppinstallkey, \
                                                      WPA_DEAUTH wpa_deauth, \
                                                      WPA_NEG_COMPLETE esp_wpa_neg_complete);

#include "pp/esf_buf.h"
void esp_eapol_txcb(esf_buf_t *eb);

void esp_wpa_set_profile(uint32 wpa_proto);

void esp_wpa_set_bss(char *macddr, char * bssid, uint8 pairwise_cipher, uint8 group_cipher, char *passphrase, u8 *ssid, size_t ssid_len);

int esp_wpa_sm_rx_eapol(u8 *src_addr, u8 *buf, u32 len);
#endif /* WPA_I_H */
