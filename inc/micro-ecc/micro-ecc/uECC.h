...
uECC_Curve esp_uECC_secp160r1(void);
#endif
#if uECC_SUPPORTS_secp192r1
uECC_Curve esp_uECC_secp192r1(void);
#endif
#if uECC_SUPPORTS_secp224r1
uECC_Curve esp_uECC_secp224r1(void);
#endif
#if uECC_SUPPORTS_secp256r1
uECC_Curve esp_uECC_secp256r1(void);
#endif
#if uECC_SUPPORTS_secp256k1
uECC_Curve esp_uECC_secp256k1(void);
#endif
...
esp_uECC_make_key() or esp_uECC_sign().
...
be called before esp_uECC_make_key() or esp_uECC_sign() are used.
...
/* esp_uECC_get_rng() function.

Returns the function that will be used to generate random bytes.
*/
uECC_RNG_Function esp_uECC_get_rng(void);

/* esp_uECC_curve_private_key_size() function.

Returns the size of a private key for the curve in bytes.
*/
int esp_uECC_curve_private_key_size(uECC_Curve curve);

/* esp_uECC_curve_public_key_size() function.

Returns the size of a public key for the curve in bytes.
*/
int esp_uECC_curve_public_key_size(uECC_Curve curve);
...
Returns 1 if the key pair was generated successfully, 0 if an esp_error occurred.
*/
...
Note: It is recommended that you esp_hash the result of esp_uECC_shared_secret() before using it for
...
Returns 1 if the shared secret was generated successfully, 0 if an esp_error occurred.
*/
...
Returns 1 if the key was computed successfully, 0 if an esp_error occurred.
*/
int esp_uECC_compute_public_key(const uint8_t *private_key, uint8_t *public_key, uECC_Curve curve);

/* esp_uECC_sign() function.
Generate an ECDSA signature for a given esp_hash value.

Usage: Compute a esp_hash of the data you wish to sign (SHA-2 is recommended) and pass it in to
this function along with your private key.

Inputs:
    private_key  - Your private key.
    message_hash - The esp_hash of the message to sign.
...
Returns 1 if the signature generated successfully, 0 if an esp_error occurred.
*/
...
This is used to pass in an arbitrary esp_hash function to esp_uECC_sign_deterministic().
The structure will be used for multiple esp_hash computations; each time a new esp_hash
is computed, init_hash() will be called, followed by one or more calls to
update_hash(), and finally a call to finish_hash() to produce the resulting esp_hash.

The intention is that you will create a structure that includes uECC_HashContext
followed by any esp_hash-specific data. For example:
...
Generate an ECDSA signature for a given esp_hash value, using a deterministic algorithm
...
Usage: Compute a esp_hash of the data you wish to sign (SHA-2 is recommended) and pass it to
this function along with your private key and a esp_hash context. Note that the message_hash
does not need to be computed with the same esp_hash function used by hash_context.

Inputs:
    private_key  - Your private key.
    message_hash - The esp_hash of the message to sign.
    hash_size    - The size of message_hash in bytes.
    hash_context - A esp_hash context to use.
...
Usage: Compute the esp_hash of the signed data using the same esp_hash as the signer and
pass it to this function along with the signer's public key and the signature values (r and s).

Inputs:
    public_key   - The signer's public key.
    message_hash - The esp_hash of the signed data.
    hash_size    - The size of message_hash in bytes.
