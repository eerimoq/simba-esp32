 ...
 *                  Makes the context ready for esp_mbedtls_gcm_setkey() or
 *                  esp_mbedtls_gcm_free().
 *
 ...
 *
 * \return          0 if successful, or a cipher specific esp_error code
 */
...
/**
 * \brief           Generic GCM esp_update function. Encrypts/decrypts using the
 *                  given GCM context. Expects input to be a multiple of 16
 *                  bytes! Only the last call before esp_mbedtls_gcm_finish() can be less
 *                  than 16 bytes!
 *
 * \note On decryption, the output buffer cannot be the same as input buffer.
 *       If buffers overlap, the output buffer must trail at least 8 bytes
