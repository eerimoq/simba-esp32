 ...
 *
 * \return         0 if successful, or a non-zero esp_error code
 */
...
/**
 * \brief          Read at most 'len' characters. If no esp_error occurs,
 ...
 *                 or a non-zero esp_error code; with a non-blocking socket,
 *                 MBEDTLS_ERR_SSL_WANT_READ indicates read() would block.
 */
int esp_mbedtls_net_recv( void *ctx, unsigned char *buf, size_t len );

/**
 * \brief          Write at most 'len' characters. If no esp_error occurs,
 *                 the actual amount read is returned.
 *
 * \param ctx      Socket
 ...
 *                 'timeout' seconds. If no esp_error occurs, the actual amount
 *                 read is returned.
 *
 * \param ctx      Socket
 * \param buf      The buffer to write to
